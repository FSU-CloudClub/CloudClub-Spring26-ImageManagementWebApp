...
const GalleryScreen = ({ user, signOut }) => {
    ...
    // Derive top 10 tags
    const tagCounts = images.reduce((acc, img) => {
        (img.Labels || []).forEach(([name]) => {
    ...
    const visibleImages = activeTag
        ? images.filter(img => img.Labels?.some(([name]) => name === activeTag))
        : images;
    ...
    const handleDelete = async (imageId) => {
        if (IS_DEMO) {
            setImages((prev) => {
                ...
                if (activeTag && !next.some(img => img.Labels?.some(([name]) => name === activeTag))) {
                    setActiveTag(null);
                }
                return next;
            });
    ...
    // -------------------------------------------------------------------------
    // Listens for updates (rather than polling)
    // -------------------------------------------------------------------------
    useEffect(() => {
        if (IS_DEMO) return;
        const handleFocus = () => handleRealRefresh();
            ...
            {!loading && topTags.length > 0 && (
                <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginBottom: '16px' }}>
                    {topTags.map(tag => (
                        <button
                            key={tag}
                            className={`btn btn-sm ${activeTag === tag ? 'btn-primary' : 'btn-outline-secondary'}`}
